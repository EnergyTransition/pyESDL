...
from esdl.profiles.datatableprofilemanager import Credentials, DataTableProfileManager
...
if __name__ == "__main__":
    ...
    dtpmanager = DataTableProfileManager.load(
        my_dtp, Credentials.create_dict(my_dtp.configuration.id, "drive", "password")
    )
    ...
    dtp3 = esdl.DataTableProfile(id="test", tableName=tableName, columnName=columnName, multiplier=multiplier)
    dtp3.configuration = esdl.DatabaseConfiguration(
        type=esdl.DatabaseTypeEnum.POSTGRESQL, id="my_database", database="energy_profiles", host="localhost", port=5432
    )
    dtpm3 = DataTableProfileManager(dtp3)
    dtpm3.add_credential(Credentials.create_dict("my_database", "postgres", "password"))
    dtpm3.load_from_database()
    ...
    dtp4.configuration = esdl.DatabaseConfiguration(
        type=esdl.DatabaseTypeEnum.POSTGRESQL, id="my_database", database="energy_profiles", host="localhost", port=5432
    )
