...
def query_data_from_postgres():
    ...
    nw_table_config = DatabaseConfiguration(name="postgres", id="postgres_db", database="datatableprofile",
                                            ...
                                            host="localhost")
    dtp.configuration = nw_table_config
    creds = Credentials.create_dict("postgres_db", "drive", "password")
    data, header, qau = DataTableProfileManager.query(dtp,
...
def write_data_to_postgres():
    ...
    dtpman.save(Credentials.create_dict("postgres_db", "drive", "password"))
